...
G4PrimaryVertex* PrimaryGenerator::GenerateFiveGammaVertex( 
  ...
  G4PrimaryVertex* vertex_five = new G4PrimaryVertex();
  VtxInformation* info = new VtxInformation();

//  G4cout<<"Vertex declaration: "<< G4endl;
  ...
  vertex_five->SetUserInformation(info);
  vertex_five->SetT0(T0 + lifetime);
  vertex_five->SetPosition(vtxPosition.x(), vtxPosition.y(), vtxPosition.z());

//  G4cout<<"Vts position: X,Y,Z: "<<vtxPosition.x()<< vtxPosition.y() <<vtxPosition.z()<<G4endl;
  ...
  if (!test) {
    G4cout << "error: generate_gamma : createFiveHitEvts:" << test << G4endl;
  }
  event.Generate();
/*
  Double_t weight = 1;
...
*/
  G4PrimaryParticle* particle_five[5];
  for (int i = 0; i < 5; i++) {
    TLorentzVector* out = event.GetDecay(i);
    particle_five[i] = new G4PrimaryParticle(
      particleDefinition, out->Px(), out->Py(), out->Pz(), out->E()
    );
 //   G4cout<<"eNERGY: "<<out->E()<<G4endl;
    ...
    particle_five[i]->SetUserInformation(infoParticle);
    vertex_five->SetPrimary(particle_five[i]); 
 
   // G4cout<<"multiplicity: "<< infoParticle->GetGeneratedGammaMultiplicity()<<G4endl;
  } 
  return vertex_five;
}
  ...
  if (decayRandom > MaterialParameters::fSodiumChanceEC + MaterialParameters::fSodiumChanceNoPrompt) {
     if (evtFractions[0] > random) {
      ...
      event->AddPrimaryVertex(GenerateFiveGammaVertex(
        DecayChannel::kOrtho5G, vtxPosition, T0,
        material->GetLifetime(random - evtFractions[0] - evtFractions[1] - evtFractions[2] - evtFractions[3] - evtFractions[4], DecayChannel::kOrtho5G)
        ));
      fDecayChannel = DecayChannel::kOrtho5G;
    }*/ else {
      // oPs 3G
      event->AddPrimaryVertex(GenerateFiveGammaVertex(
        DecayChannel::kOrtho5G, vtxPosition, T0,
